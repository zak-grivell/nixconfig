...
function Aerospace:focused_window(callback)
    ...
    self:send(query)
    local response = decode_response(self:receive(DEFAULT_CONFIG.MAX_BUFFER_SIZE))
    local focused = decode(response)


    if callback then
        return callback(focused)
    end
    return response
end
...
function Aerospace:list_all_windows(callback)
    local query = {
        command = "",
        args = { "list-windows", "--all", "--json", "--format", "%{window-id}%{app-name}%{window-title}%{workspace}%{app-bundle-id}" },
        stdin = ""
    }
